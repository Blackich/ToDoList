...
function otrisovkaEbat(arrValues) {

    if (tasks.length > 0) {

        tasks.forEach((btnSave, index) => {
            output.insertAdjacentHTML('afterbegin', outputTemplateTaTaTavoRot(btnSave, index));
        });
    }
}
...
function outputTemplateTaTaTavoRot(btnSave, index) {
    ...
    for (let btnSave of btnsSave) {
        ...
        if (!tasks[btnSave.parentElement.id]) {
            ...
            if (btnSave.parentElement.firstElementChild.textContent) {
                ...
                btnSave.parentElement.firstElementChild.textContent = tasks[btnSave.parentElement.id].text; //text = tasks[i]
            ...
            if (btnSave.parentElement.firstElementChild.textContent) {
                ...
                tasks[btnSave.parentElement.id].text = btnSave.parentElement.firstElementChild.textContent;   //tasks[i] = text
                updateLocal();
...
document.addEventListener('click', (e) => {
    ...
    for (let btnCancel of btnsCancel) {
...
document.addEventListener('click', (e) => {
    ...
    for (let btnChange of btnsChange) {
